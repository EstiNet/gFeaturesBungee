...
public class WarnCommand extends EstiCommand {
    ...
    public void execute(CommandSource sender, String[] args) {
        if (args.length < 3) {
            sender.sendMessage(EstiBans.estiBansPrefix.append(Component.text("/warn [Player] [Length] [Reason]")));
        } else {
            try {
                String reason = "";
                ...
                sender.sendMessage(EstiBans.estiBansPrefix.append(Component.text("Warned player " + args[0] + " for " + args[1] + " on " + args[2] + " because of \"" + reason + "\"")));
            } catch (Exception e) {
                if (gFeatures.debug) {
                    e.printStackTrace();
                }
